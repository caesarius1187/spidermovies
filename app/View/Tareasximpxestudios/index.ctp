<div class="tareasximpxestudios index">
	<h2><?php echo __('Tareasximpxestudios'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('descripcion'); ?></th>
			<th><?php echo $this->Paginator->sort('tareasimpuesto_id'); ?></th>
			<th><?php echo $this->Paginator->sort('estado'); ?></th>
			<th><?php echo $this->Paginator->sort('estudio_id'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($tareasximpxestudios as $tareasximpxestudio): ?>
	<tr>
		<td><?php echo h($tareasximpxestudio['Tareasximpxestudio']['id']); ?>&nbsp;</td>
		<td><?php echo h($tareasximpxestudio['Tareasximpxestudio']['descripcion']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($tareasximpxestudio['Tareasimpuesto']['id'], array('controller' => 'tareasimpuestos', 'action' => 'view', $tareasximpxestudio['Tareasimpuesto']['id'])); ?>
		</td>
		<td><?php echo h($tareasximpxestudio['Tareasximpxestudio']['estado']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($tareasximpxestudio['Estudio']['id'], array('controller' => 'estudios', 'action' => 'view', $tareasximpxestudio['Estudio']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $tareasximpxestudio['Tareasximpxestudio']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $tareasximpxestudio['Tareasximpxestudio']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $tareasximpxestudio['Tareasximpxestudio']['id']), null, __('Are you sure you want to delete # %s?', $tareasximpxestudio['Tareasximpxestudio']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Tareasximpxestudio'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Tareasimpuestos'), array('controller' => 'tareasimpuestos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Tareasimpuesto'), array('controller' => 'tareasimpuestos', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Estudios'), array('controller' => 'estudios', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Estudio'), array('controller' => 'estudios', 'action' => 'add')); ?> </li>
	</ul>
</div>
